...
export default function TermsOfServicePage() {
  ...
  useEffect(() => {
    ...
    policiesApi
      ...
      .catch(() => {
        if (alive) {
          toast.error("Không thể tải điều khoản dịch vụ");
        }
      })
      .finally(() => {
        if (alive) {
        ...
        <div className="overflow-hidden rounded-3xl border border-black/5 bg-white p-6 shadow-sm sm:p-10 md:p-12">
          {loading ? (
            ...
            <article className="prose prose-slate max-w-none">
              <div className="text-sm leading-7 text-slate-600 sm:text-base sm:leading-8" dangerouslySetInnerHTML={{ __html: terms }} />
            </article>
          )}
        </div>
      </div>
