...
const uniqueMediaDrafts = (items: MediaDraft[]) => {
...
const buildCategoryOptions = (items: Category[]) => {
  ...
  const sortItems = (list: Category[] = []) =>
    [...list].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0) || a.name.localeCompare(b.name));

  const walk = (parentId: number | null, depth = 0): Array<{ category: Category; depth: number }> => {
...
export default function AdminProductFormPage() {
  ...
  const sortedCategoryOptions = buildCategoryOptions(categories);
  ...
  useEffect(() => {
    ...
    productsApi.getById(Number(id))
      .then((r) => {
        ...
        const loadedMedia = uniqueMediaDrafts((p.media ?? p.images ?? [])
          .filter((img) => Boolean(getProductMediaUrl(img)))
          .map((img, idx) => ({
            localId: `media-${img.id ?? idx}`,
            url: toFrontendImageUrl(getProductMediaUrl(img)),
            type: img.type === "video" ? "video" : inferMediaTypeFromUrl(getProductMediaUrl(img)),
          })));
  ...
  const addToGallery = (url: string) => {
    const safeUrl = toFrontendImageUrl(url);
    if (!safeUrl) return;
    setMedia((curr) => {
  ...
  const handleGalleryDrop = async (files: FileList | File[]) => {
    ...
    try {
      const uploaded = mediaFiles.length === 1
        ? [{ url: toFrontendImageUrl(await uploadOne(mediaFiles[0])), type: inferMediaTypeFromFile(mediaFiles[0]) }]
        : await Promise.all(mediaFiles.map(async (file) => ({
            url: toFrontendImageUrl(await uploadOne(file)),
  ...
  const uploadColorImage = async (vi: number, ci: number, file?: File) => {
    if (!file) return;
    try {
      const url = toFrontendImageUrl(await uploadOne(file));
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <SelectContent>
                  {sortedCategoryOptions.map(({ category: c, depth }) => (
                    <SelectItem key={c.id} value={String(c.id)}>
                      {`${"↳ ".repeat(depth)}${c.sortOrder}. ${c.name}`}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                {media
                  ...
                  >
                    {item.type === "video" ? (
                      ...
                      <img src={item.url.trim()} alt={`Ảnh ${index + 1}`} className="h-full w-full object-cover" />
                    )}
                          ...
                          <div className="space-y-1">
                            ...
                            {(() => {
                              ...
                              <div className="relative h-16 rounded-lg overflow-hidden border group">
                                <img src={sharedImageUrl} alt={color.color || "color"} className="h-full w-full object-cover" />
                                <button type="button"
